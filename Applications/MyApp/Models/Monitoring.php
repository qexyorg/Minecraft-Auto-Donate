...
use Alonity\Components\File;

class MonitoringModelException extends \Exception {}
...
class MonitoringModel {
	...
	public function getMonitoringJson(){

		$time = time();

		if($time > ($this->app['server']['updated']+$this->app['server']['cache'])){
			$stats = $this->getStats($this->app['server']['ip']);

			$this->app['server']['status'] = $stats['status'];
			$this->app['server']['online'] = $stats['online'];
			$this->app['server']['slots'] = $stats['slots'];
			$this->app['server']['updated'] = $time;

			if($stats['online']>$this->app['server']['record']){
				$this->app['server']['record'] = $stats['online'];
			}

			if(date("d", $this->app['server']['updated'])!=date("d")){
				$this->app['server']['today'] = $stats['online'];
			}

			if($this->app['server']['today']<$stats['online']){
				$this->app['server']['today'] = $stats['online'];
			}

			$build = File::config()
				->setPath($this->alonity->getRoot().'/Applications/MyApp')
				->setData($this->app)
				->build()
				->name('MyApp');
			...
			if(!$build->execute()){
				return [
					'type' => false,
					'text' => 'Произошла ошибка обновления мониторинга'
				];
			}
		}
		...
		return [
			'type' => true,
			'status' => $this->app['server']['status'],
			'online' => $this->app['server']['online'],
			'slots' => $this->app['server']['slots'],
			'record' => $this->app['server']['record'],
			'today' => $this->app['server']['today']
		];
	}
}
